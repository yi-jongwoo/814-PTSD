...
__global__ void addKernel(gene* crr, ull gseed) {
	...
	if (tid < 2048) {
		...
		for (int round = 0; round <= 32; round++) {
			...
			for (int t = 0; t < 16; t++) {
				memcpy(nextpool[t], pool[t], sizeof(gene));
				int x1 = rnd(rseed) % 8, x2 = rnd(rseed) % 8, x3 = rnd(rseed) % 8, x4 = rnd(rseed) % 8 ;
				int y1 = rnd(rseed) % 14, y2 = rnd(rseed) % 14, y3 = rnd(rseed) % 14, y4 = rnd(rseed) % 14;
				nextpool[t][x1][y1] = rnd(rseed) % 10;
				nextpool[t][x2][y2] = rnd(rseed) % 10;
				nextpool[t][x3][y3] = rnd(rseed) % 10;
				nextpool[t][x4][y4] = rnd(rseed) % 10;
				if (geval(nextpool[t]) >= (epool[t] >> 16))
					memcpy(pool[t], nextpool[t], sizeof(gene));
			}
		}
	}
}
...
int main() {
	...
	mt19937 rgen; rgen.seed(7544845);
	for (int t = 0; t < 4096; t++)
		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 14; j++)
				crr[t][i][j] = rgen()%10;
	char(*ptmp)[14] = (char(*)[14])
		"5237509986374196236437436183498198215298143705728519755450235406406726841649372406123871681378350052092041598399";
	for(int t=0;t<1024;t++)
		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 14; j++)
				crr[t][i][j] = ptmp[i][j] - '0';
	ptmp = (char(*)[14])
		"0235509976370306236437436182498198215298493705728519753950235406406724841649372406124871681378350751928041569356";
	for (int t = 1024; t < 2048; t++)
		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 14; j++)
				crr[t][i][j] = ptmp[i][j] - '0';
	ptmp = (char(*)[14])
		"8235509976370396236437436182498198215298493705728519753950235406406724841649372406124871681378350651928041569357";
	for (int t = 2048; t < 3072; t++)
		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 14; j++)
				crr[t][i][j] = ptmp[i][j] - '0';
	ptmp = (char(*)[14])
		"3237509986379196236437436183498198215298143705728519755450235406406726841649372406123871681378350952092041598399";
	for (int t = 3072; t < 4096; t++)
		for (int i = 0; i < 8; i++)
			for (int j = 0; j < 14; j++)
				crr[t][i][j] = ptmp[i][j] - '0';
	int rrr = 0;
